...
Please run the following command before running the script to get the unwrapping result of the Sentinel-1 Processor (https://sarimggeodesy.github.io/software/):

LiCSBAS02_unwrapByGAMMA.py -in dint_geocode.tif -i "your_intf_list_folder"

****************

LiCSBAS02_ml_prep_InSARProcessorGeocoded.py.py -i GEOCdir [-o GEOCmldir] [-n nlook] [--freq float] [--n_para int]
...
unwName = 'pha_geocode.tif'
cohName = 'coh_geocode.tif'
...
def main(argv=None):
    ...
    if n_ifg2 > 0:
        ...
        if ifgd_ok:
            ...
            # unw_tiffile = os.path.join(geocdir, ifgd_ok, unwName)
            unw_tiffile = os.path.join(geocdir, ifgd_ok, 'dint_geocode.tif')
...
def convert_wrapper(i):
    ...
    unw_tiffile = os.path.join(geocdir, ifgd, unwName)
    cc_tiffile = os.path.join(geocdir, ifgd, cohName)

    ### Check if inputs exist
    if not os.path.exists(unw_tiffile):
        print ('  No {} found. Skip'.format(ifgd+'/'+unwName), flush=True)
        return 1
    elif not os.path.exists(cc_tiffile):
        print ('  No {} found. Skip'.format(ifgd+'/'+cohName), flush=True)
        return 1
    ...
    try:
        ...
        print ('  {} cannot open. Skip'.format(ifgd+'/'+unwName), flush=True)
    ...
    try:
        ...
        print ('  {} cannot open. Skip'.format(ifgd+'/'+cohName), flush=True)
        shutil.rmtree(ifgdir1)
        return 1
