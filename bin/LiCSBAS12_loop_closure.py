...
def main(argv=None):
    ...
    global Aloop, ifgdates, ifgdir, length, width, loop_pngdir, cycle, multi_prime, bad_ifg, noref_ifg, bad_ifg_all, refy1, refy2, refx1, refx2 ## for parallel processing
    ...
    loop_ph_rms_ifg = np.array(p.map(loop_closure_1st_wrapper, [(i, Aloop, ifgdates, ifgdir, length, width, loop_pngdir, cycle, multi_prime) for i in range(n_loop)]), dtype=np.float32)
    ...
    res = np.array(p.map(loop_closure_2nd_wrapper, [(i,Aloop, ifgdates, ifgdir, length, width, loop_pngdir, cycle, multi_prime, bad_ifg) for i in args] ), dtype=np.float32)
    p.close()
    ...
    loop_ph_rms_ifg2 = list(np.array(p.map(loop_closure_3rd_wrapper, [(i,Aloop, ifgdates, ifgdir, length, width, loop_pngdir, cycle, multi_prime, bad_ifg, noref_ifg,refy1, refy2, refx1, refx2) for i in range(n_loop)]), dtype=np.float32))
    p.close()
    ...
    res = np.array(p.map(loop_closure_4th_wrapper, [(i,Aloop, ifgdates, ifgdir, length, width, loop_pngdir, cycle, multi_prime, bad_ifg, noref_ifg, bad_ifg_all, refy1, refy2, refx1, refx2) for i in args] ), dtype=np.int16)
    p.close()
...
def loop_closure_1st_wrapper(i):
    i,Aloop, ifgdates, ifgdir, length, width, loop_pngdir, cycle, multi_prime=i
    n_loop = Aloop.shape[0]
...
def loop_closure_2nd_wrapper(args):
    args, Aloop, ifgdates, ifgdir, length, width, loop_pngdir, cycle, multi_prime, bad_ifg = args
    i0, i1 = args
    n_loop = Aloop.shape[0]
...
def loop_closure_3rd_wrapper(i):
    i,Aloop, ifgdates, ifgdir, length, width, loop_pngdir, cycle, multi_prime, bad_ifg, noref_ifg,refy1, refy2, refx1, refx2=i
    n_loop = Aloop.shape[0]
...
def loop_closure_4th_wrapper(args):
    args, Aloop, ifgdates, ifgdir, length, width, loop_pngdir, cycle, multi_prime, bad_ifg, noref_ifg, bad_ifg_all, refy1, refy2, refx1, refx2 = args
    i0, i1 = args
    n_loop = Aloop.shape[0]
